...
#define cstr(s)                                                                                                        \
    {                                                                                                                  \
        .bytes = (s), .len = ARRAYSIZE(s) - 1                                                                          \
    }
inline bool
cstr_eq(const char* a, const char* b)
{
    return strcmp(a, b) == 0;
}
inline bool
mem_eq(const void* a, const void* b, size_t len)
{
...
{
    return a.len == b.len && mem_eq(a.bytes, b.bytes, a.len);
}
...
#define SPAN_FIRST(span, cnt)                                                                                          \
    {                                                                                                                  \
        .data = (span).data, .count = min((span).count, cnt),                                                          \
    }

DEF_SPAN_T(char, char_span_t);

char_span_t
get_first_split(char_span_t buffer, char split_char);

char_span_t
get_next_split(const char_span_t current_split, const char_span_t full_span, char split_char);

#define split_by(span_t, iter_var_name, buffer, split_char)                                                            \
    for (span_t iter_var_name = get_first_split((buffer), (split_char)); iter_var_name.data != NULL;                   \
